...
Image decodeImage(const std::vector<uint8_t>& data) {
    ...
    if (!(data[0]=='N' && data[1]=='C' && data[2]=='0' && data[3]=='9'))
        throw std::runtime_error("decodeImage: bad magic (expect NC09)");
    ...
    const int origW = (int)rd32(4);
    const int origH = (int)rd32(8);
    // ★ الأبعاد المبطّنة (مضاعفات 64)
    const int W = ((origW + 63) / 64) * 64;
    const int H = ((origH + 63) / 64) * 64;

    const int quality = (int)data[12];
    const int C = (int)data[13];
    if (origW <= 0 || origH <= 0) throw std::runtime_error("decodeImage: bad size");
    ...
    // ★ قص الصورة للأبعاد الأصلية
    if (W != origW || H != origH) {
        Image cropped;
        cropped.width    = origW;
        cropped.height   = origH;
        cropped.channels = img.channels;
        cropped.pixels.resize((size_t)origW * origH * img.channels);
        for (int y = 0; y < origH; y++)
            for (int x = 0; x < origW; x++)
                for (int c = 0; c < img.channels; c++)
                    cropped.pixels[((size_t)y * origW + x) * img.channels + c] =
                        img.pixels[((size_t)y * W + x) * img.channels + c];
        return cropped;
    }
    return img;
}
